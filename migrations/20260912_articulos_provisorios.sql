-- Artículos provisorios: identidad histórica sin crear productos ni alterar stock.
-- Ejecutar después de 20260911_procesar_venta_atomica.sql.
BEGIN;

ALTER TABLE public.ventas
    ADD COLUMN IF NOT EXISTS tiene_provisorios BOOLEAN NOT NULL DEFAULT false;
ALTER TABLE public.ventas_detalle
    ADD COLUMN IF NOT EXISTS es_provisorio BOOLEAN NOT NULL DEFAULT false,
...
DECLARE
    ...
    v_item JSONB;
    v_prod_id BIGINT;
    v_es_provisorio BOOLEAN;
    v_tiene_provisorios BOOLEAN := false;
    v_descripcion TEXT;
    v_cantidad NUMERIC;
    v_p_unit NUMERIC;
    v_p_contado NUMERIC;
...
BEGIN
    ...
    LOOP
        ...
        v_es_provisorio := COALESCE((v_item->>'es_provisorio')::BOOLEAN, false);
        IF v_es_provisorio THEN
            v_descripcion := btrim(v_item->>'descripcion');
            v_costo_unit := COALESCE((v_item->>'costo_unitario')::NUMERIC, 0);
            IF NULLIF(v_item->>'producto_id', '') IS NOT NULL
                OR NULLIF(v_item->>'promocion_id', '') IS NOT NULL THEN
                RAISE EXCEPTION 'Un artículo provisorio no puede estar asociado a un producto o promoción.';
            END IF;
            IF v_descripcion IS NULL OR v_descripcion = '' THEN
                RAISE EXCEPTION 'El artículo provisorio debe tener un nombre.';
            END IF;
            IF v_cantidad IS NULL OR v_cantidad <= 0
                OR v_cantidad::TEXT IN ('NaN', 'Infinity', '-Infinity')
                OR v_p_unit IS NULL OR v_p_unit < 0
                OR v_p_unit::TEXT IN ('NaN', 'Infinity', '-Infinity')
                OR v_p_contado IS NULL OR v_p_contado < 0
                OR v_p_contado::TEXT IN ('NaN', 'Infinity', '-Infinity')
                OR v_costo_unit < 0
                OR v_costo_unit::TEXT IN ('NaN', 'Infinity', '-Infinity') THEN
                RAISE EXCEPTION 'El artículo provisorio requiere cantidad positiva y precios y costo válidos, no negativos.';
            END IF;
            v_tiene_provisorios := true;
        END IF;
    ...
    LOOP
        ...
        v_p_contado := COALESCE((v_item->>'precio_contado')::NUMERIC, v_p_unit);
        v_es_provisorio := COALESCE((v_item->>'es_provisorio')::BOOLEAN, false);
        v_descripcion := CASE WHEN v_es_provisorio THEN btrim(v_item->>'descripcion') ELSE NULL END;
        v_costo_unit := CASE WHEN v_es_provisorio THEN COALESCE((v_item->>'costo_unitario')::NUMERIC, 0) ELSE 0 END;
        ...
        IF NOT v_es_provisorio AND v_prod_id IS NOT NULL THEN
